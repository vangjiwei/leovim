...
if exists('*matchstrpos')
    ...
    nnoremap s<Cr> :GrepperSearchAll <C-r><C-w>
    ...
    if g:complete_engine != 'coc'
        nnoremap <silent><M-?> :GrepperSearchAll <C-r><C-w><CR>
    endif
    " GrepperSearch
    nnoremap S<cr> :GrepperSearch <C-r><C-w>
...
if Installed('leaderf')
    if exists('g:search_tool')
        ...
        nnoremap s<Cr> :Leaderf --stayOpen rg -L -S "<C-r><C-w>"<CR>
    endif
    let g:Lf_RgConfig = [
        \ "--max-columns=10000",
        \ "--glob=!git/*",
    ...
    nnoremap <leader>? :Leaderf rg -L -S <C-r><C-w>
    ...
    nnoremap <C-f>f :Leaderf --stayOpen rg -L -S -F "<C-r><C-w>"
    xnoremap <C-f>x :<C-u>Leaderf --stayOpen rg -L -S -x "<C-r>=GetVisualSelection()<CR>"
    nnoremap <C-f>x :Leaderf --stayOpen rg -L -S -x "<C-r><C-w>"
    xnoremap <C-f>a :<C-u>Leaderf --stayOpen rg --append "<C-r>=GetVisualSelection()<CR>"
    nnoremap <C-f>a :Leaderf --stayOpen rg --append "<C-r><C-w>"
    " interactive
...
elseif InstalledTelescope()
    ...
    nnoremap <leader>? :TeleSearch <C-r><C-w>
...
elseif get(g:, 'search_tool', '') == 'grepper'
    ...
    nnoremap <leader>? :GrepperSearch <C-r><C-w>
...
if Installed('nvim-bqf')
    luafile $LUA_PATH/bqf-config.lua
    ...
    au Filetype qf nnoremap <silent><buffer> <C-v> <Cr><C-w>z:vsplit<Cr>
    ...
    if Installed('vim-quickui')
        ...
        " preview
        au FileType qf nnoremap <silent><buffer> <C-h> :call quickui#tools#preview_quickfix()<cr>
